...
def grouped_pad(tensor_groups, dims, values):
    paddings = [MAX_TOTAL_TOKENS -
                tensors[0].size(dim) if dim is not None else 0 for tensors, dim in zip(tensor_groups, dims)]
...
def grouped_extend_batch(tensor_groups, target_bs, bs_dims):
    for tensors, bs_dim in zip(tensor_groups, bs_dims):
        assert bs_dim in [0, 1], 'Batch dims other than 0 or 1 are not supported!'
        for i in range(len(tensors)):
            diff = target_bs - tensors[i].size(bs_dim)
            # TODO: add support for shrinking bs
            if diff > 0:
                prev = tensors[i]
                shape = list(prev.shape)
                shape[bs_dim] = target_bs
                tensors[i] = torch.empty(shape, device=prev.device, dtype=prev.dtype)
                for batch_idx in range(prev.size(bs_dim)):
                    if bs_dim == 0:
